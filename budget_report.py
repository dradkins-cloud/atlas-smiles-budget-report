...
# Sequence account name â email display name.
...
# ââ Sequence API âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    for attempt in range(max_retries + 1):
        ...
        if resp.status_code == 429:
            if attempt < max_retries:
                ...
                print(f"  Rate limited â waiting {wait}s before retry {attempt + 1}...")
                time.sleep(wait)
                continue
...
def build_account_rows(pod_accounts):
    ...
    for seq_name, display_name in ACCOUNT_MAP:
        ...
        if not account:
            print(f"  Warning: account not found in Sequence â '{seq_name}'")
            rows.append({
...
def _fmt_date(iso_str):
    """Parse ISO 8601 UTC timestamp â 'Mon D' (e.g. 'May 20')."""
    if not iso_str:
        return "â"
...
def _fmt_dollars(cents):
    """Convert integer cents â formatted dollar string (e.g. '$1,204.50')."""
    if cents is None:
        return "â"
...
# ââ HTML construction ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    if not found:
        return f"""
          <tr style="background-color:{alt_bg};">
            ...
            <td style="padding:11px 14px;border-bottom:1px solid #eaecef;text-align:right;color:#aaaaaa;">â</td>
            <td style="padding:11px 14px;border-bottom:1px solid #eaecef;text-align:center;color:#aaaaaa;">â</td>
          ...
          <tr>
            <td style="padding:12px 16px;font-size:14px;color:#3a4a7a;">
              * Balance data for <strong>{names}</strong> may not reflect the
              latest transactions â Sequence had not yet refreshed these accounts
              at report time.
            </td>
          </tr>
        </table>"""
          ...
          <tr>
            <td style="background-color:#30AED6;padding:14px 32px;">
              <p style="margin:0;color:#ffffff;font-size:16px;font-weight:700;letter-spacing:0.5px;">
                BUDGET UPDATE &nbsp;Â·&nbsp; {day_of_week}, {today_str.upper()}
              </p>
            </td>
          </tr>
...
def send_email(html_body):
    ...
    subject = f"Atlas Smiles â Budget Update (Week of {week_of})"
    ...
    print(f"Sent: '{subject}' â {', '.join(RECIPIENTS)}")


# ââ Entry point ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

def main():
    print(f"Atlas Smiles budget report â {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
